...
###Changepoint analysis of major renewable energy series.
...
#### Determining the linear trend of the solar series
```{r}
#Lm trend for solar series section 1
Nat.Solar.1.t <- 1:130
Nat.Solar.1.y <- deseasonal_Nat.Solar[1:130]
lm.Nat.Solar.1 <- lm(Nat.Solar.1.y~Nat.Solar.1.t)
summary(lm.Nat.Solar.1)
```

> Based on the results of the linear regression (F-statistic: 1.238 on 1 and 129 DF, p-value: 0.2679) there is no significant change in the electricity generation from solar energy before 2011-10-01

```{r}
#Lm trend for solar series section 2
Nat.Solar.2.t <- 131:170
Nat.Solar.2.y <- deseasonal_Nat.Solar[131:170]
lm.Nat.Solar.2 <- lm(Nat.Solar.2.y~Nat.Solar.2.t)
summary(lm.Nat.Solar.2)
```

> Based on the results of the linear regression (F-statistic: 238.1 on 1 and 38 DF,  p-value: < 2.2e-16) there is a significant change in the electricity generation from solar energy between 2011-10-01 and 2015-02-01 (between change point 1 and 2) with an increasing gradient of 43200 MWh for every month increase in time.

```{r}
#Lm trend for solar series section 3
Nat.Solar.3.t <- 171:194
Nat.Solar.3.y <- deseasonal_Nat.Solar[171:194]
lm.Nat.Solar.3 <- lm(Nat.Solar.3.y~Nat.Solar.3.t)
summary(lm.Nat.Solar.3)
```

> Based on the results of the linear regression (F-statistic: 15.39 on 1 and 22 DF,  p-value: 0.0007285) there is a significant change in the electricity generation from solar energy between 2015-02-01 and 2017-02-01 (between changepoint 2 and 3) with an increasing gradient of 53886 MWh for every month increase in time.

```{r}
#Lm trend for solar series section 4
Nat.Solar.4.t <- 195:217
Nat.Solar.4.y <- deseasonal_Nat.Solar[195:217]
lm.Nat.Solar.4 <- lm(Nat.Solar.4.y~Nat.Solar.4.t)
summary(lm.Nat.Solar.4)
```

> Based on the results of the linear regression (F-statistic: 0.3116 on 1 and 21 DF,  p-value: 0.5826) there is no significant change in the electricity generation from solar energy between 2017-02-01 to 2019-01-01 (after changepoint 3)
...
#### Determining the linear trend of the wind series
```{r}
#Lm trend for wind series section 1
Nat.Wind.1.t <- 1:110
Nat.Wind.1.y <- deseasonal_Nat.Wind[1:110]
lm.Nat.Wind.1 <- lm(Nat.Wind.1.y~Nat.Wind.1.t)
summary(lm.Nat.Wind.1)
```

> Based on the results of the linear regression (F-statistic: 205.2 on 1 and 108 DF,  p-value: < 2.2e-16) there is a significant change in the electricity generation from wind energy before chnage point 1 in 2010-02-01. There is an increase of 55251 (MWh) generation for every month increase in time.

```{r}
#Lm trend for wind series section 2
Nat.Wind.2.t <- 111:166
Nat.Wind.2.y <- deseasonal_Nat.Wind[111:166]
lm.Nat.Wind.2 <- lm(Nat.Wind.2.y~Nat.Wind.2.t)
summary(lm.Nat.Wind.2)
```

> Based on the results of the linear regression (F-statistic: 195.1 on 1 and 54 DF,  p-value: < 2.2e-16) there is a significant change in the electricity generation from wind energy between 2010-02-01 and 2014-10-01 (between change point 1 and 2) with an increase of 142540 MWh for every month increase in time.

```{r}
#Lm trend for wind series section 3
Nat.Wind.3.t <- 167:189
Nat.Wind.3.y <- deseasonal_Nat.Wind[167:189]
lm.Nat.Wind.3 <- lm(Nat.Wind.3.y~Nat.Wind.3.t)
summary(lm.Nat.Wind.3)
```

> Based on the results of the linear regression (F-statistic: 10.41 on 1 and 21 DF,  p-value: 0.00404) there is a significant change in the electricity generation from wind energy between 2014-10-01 and 2016-09-01 (between changepoint 2 and 3) with an increasing gradient of 180069 MWh for every month increase in time.

```{r}
#Lm trend for wind series section 4
Nat.Wind.4.t <- 189:217
Nat.Wind.4.y <- deseasonal_Nat.Wind[189:217]
lm.Nat.Wind.4 <- lm(Nat.Wind.4.y~Nat.Wind.4.t)
summary(lm.Nat.Wind.4)
```

> Based on the results of the linear regression (F-statistic: 3.062 on 1 and 27 DF,  p-value: 0.09149) there is no significant change in the electricity generation from solar energy between 2016-09-01 to 2019-01-01 (after changepoint 3)
